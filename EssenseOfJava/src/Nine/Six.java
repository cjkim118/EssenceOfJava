...
public class Six {
	public static String fillZero(String src, int length) {
		...
		*/
		...
		if(src.length() == length || src == null) return src;
		if(length <= 0) return "";
		if(length < src.length()) return src.substring(0,length);
		char[] result = new char[length];
		for(int i=0; i< length - src.length();i++){
			result[i] = '0';
		}
		for(int i= length - src.length();i < length;i++){
			result[i] = src.charAt(i-(length - src.length())); //System.arrayCopy또한 사용 가능 !
		}
